  ...
  submitExpense() {
    if (this.expenseForm.valid) {
      if (this.monthlyBudget === 0) {
        this.toastr.error('Please set a monthly budget before adding expenses.', 'Error');
        return;
      }
...
loadBudgetAndCurrency() {
  ...
  if (savedBudgetData) {
    try {
      ...
      this.selectedCurrency = selectedCurrency || { symbol: '$', name: 'USD' };
      this.monthlyBudgetValue = this.monthlyBudget;

      // Find the corresponding currency object from the currencies array
      this.selectedCurrencyValue = this.currencies.find(
        currency => currency.symbol === this.selectedCurrency.symbol && currency.name === this.selectedCurrency.name
  ...
  calculateSpendingPercentage() {
    debugger
    this.totalSpent = this.expenses.reduce((acc, expense) => acc + expense.totalAmount, 0);
    this.spendingPercentage = (this.totalSpent / this.monthlyBudget) * 100 || 0;
  }

  getSpendingColor(): string {
    const currentSpending = this.expenses.reduce((acc, expense) => acc + expense.totalAmount, 0);
